...
async function loadMailSettings() {
  ...
  const rawSecure = readEnv("SMTP_SECURE", "MAIL_SECURE") || row.smtp_secure;
  return {
    host: readEnv("SMTP_HOST", "MAIL_HOST") || row.smtp_host || "smtp.mx.cloudflare.net",
    port,
    secure: String(rawSecure || "").toLowerCase() === "true" || port === 465,
...
async function logMail({ recipient, subject, status, errorMessage = "", source = "crm" }) {
  try {
    await rest("mail_logs", {
      ...
      body: JSON.stringify({ recipient, subject, status, error_message: errorMessage ? String(errorMessage).slice(0, 500) : null, source, sent_at: new Date().toISOString() }),
    });
  } catch (_error) {}
}
...
async function handleMailCrm(req, res, action) {
  ...
  if (action === "send-mail") {
    if (req.method !== "POST") return json(res, 405, { ok: false, success: false, message: "Method not allowed", error: "Method not allowed", code: "METHOD_NOT_ALLOWED" });
    const body = parseBody(req);
...
function sendBackendScript(res) {
  ...
  res.status(200).send(`
    ...
    window.SIDYA_BUILD = ${JSON.stringify(build)};
    (function(){
      ...
      function loadSiteEnhancements(){ renderBuildInfo(); if (!/\\/admin(?:\\.html)?$/.test(location.pathname)) { appendStyle("sidyaAiAssistantStyles", "/sidya-ai-assistant.css?v=20260717-1"); appendScript("sidyaAiAssistantScript", "/sidya-ai-assistant.js?v=20260717-1"); } if (document.getElementById("quoteForm")) { appendScript("sidyaMailCrmRouteShimScript", "/mail-crm-route-shim.js?v=20260711-1"); appendScript("sidyaSiteMailCrmScript", "/site-mail-crm.js?v=20260711-1"); } }
      function loadFixes(){ loadSiteEnhancements(); if (!appReady()) return; appendScript("sidyaMailCrmRouteShimScript", "/mail-crm-route-shim.js?v=20260711-1"); appendScript("sidyaAdminProfitFixScript", "/admin-profit-fix.js?v=20260705-1"); appendScript("sidyaAdminProfitTableV4Script", "/admin-profit-table-v4.js?v=20260706-1"); appendScript("sidyaInfoActionsScript", "/admin-info-actions.js?v=20260706-1"); appendScript("sidyaMailCrmAdminScript", "/admin-mail-crm.js?v=20260711-1"); appendScript("sidyaMailSmtpFixScript", "/admin-mail-smtp-fix.js?v=20260712-2"); appendScript("sidyaAiAdminScript", "/admin-ai-assistant.js?v=20260721-ai-delete-1"); }
