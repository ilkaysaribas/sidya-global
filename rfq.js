(function () {
  ...
  function mergeCatalog(products) {
    var seen = new Set(catalog.map(productKey));
    (products || []).forEach(function (product) {
      var key = productKey(product);
      if (key && !seen.has(key)) { seen.add(key); catalog.push(product); }
    });
  }
  async function loadCatalog() {
    try {
      var response = await fetch("/api/rfq-products", { cache: "no-store" });
      var data = await response.json();
      if (!response.ok) throw new Error(data.error || "Ürün listesi alınamadı.");
      mergeCatalog(data.products || []);
      catalogLoaded = true;
      renderProducts(els.search ? els.search.value : "");
      addProductFromUrl();
    } catch (error) {
      catalogLoaded = true;
      if (!catalog.length && els.results) els.results.innerHTML = '<p class="rfq-note">' + escapeHtml(error.message) + '</p>';
    }
  }
  ...
  function addProductFromUrl() {
    ...
    var product = catalog.find(function (item) { return normalize([productKey(item), productName(item), item.sku, item.barcode, item.brand].join(" ")).includes(wanted); });
  ...
  function renderProducts(term) {
    ...
    if (!catalog.length && !catalogLoaded) { els.results.innerHTML = '<p class="rfq-note">' + t('loading') + '</p>'; return; }
  ...
  async function submitRfq(event) {
    ...
    if (!state.items.length) return setError(t('addAtLeastOne'));
    try {
  ...
  function bind() {
    ...
    els.results.addEventListener("click", function (event) { var id = event.target.getAttribute("data-add-product"); if (!id) return; var product = catalog.find(function (item) { return productKey(item) === id; }); if (product) addProduct(product); });
    ...
    loadCart(); renderProducts(""); renderLines(); loadCatalog();
  }
