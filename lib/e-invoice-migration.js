...
const verifySql = `
select
  to_regclass('public.einvoice_settings') is not null as einvoice_settings,
  to_regclass('public.einvoice_drafts') is not null as einvoice_drafts,
  to_regclass('public.einvoice_events') is not null as einvoice_events,
  to_regclass('public.einvoice_outbox') is not null as einvoice_outbox;
`;
...
module.exports = async (req, res) => {
  try {
    ...
    const result = await runManagementSql(verifyOnly ? verifySql : migrationSql);
    const verify = await runManagementSql(verifySql);
    return json(res, 200, { ok: true, ranMigration: !verifyOnly, result, verify });
  } catch (error) {
    console.error("E-invoice migration failed", { code: error.code || "SERVER_ERROR", statusCode: error.statusCode || 500, details: error.safeDetails || null });
    return json(res, error.statusCode || 500, {
      ok: false,
      error: error.message || "E-fatura migration çalıştırılamadı.",
